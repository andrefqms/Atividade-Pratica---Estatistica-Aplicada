---
title: "Atividade Pratica 7 de Estatistica Aplicada"
author: "André Filipe Queiroz de Melo e Soares"
date: "7 Novembro de 2018"
output:
...
print(1 - pchisq(3.593749,4))

#Para = 0,05, como P ~= 0,975 > 0,05, não há evidências para rejeitarmos a hipótese H, isto é, ao nível de significância de 5%.
```
...
inst = prop.table(table(Instrucao))
total3 <- length(Instrucao)
sal = prop.table(table(salario))
conjunta = data.frame(proc,inst)
```
...
qui_quadrado = chisq.test(estado_civil,grau_instrucao)
print(qui_quadrado)
```
